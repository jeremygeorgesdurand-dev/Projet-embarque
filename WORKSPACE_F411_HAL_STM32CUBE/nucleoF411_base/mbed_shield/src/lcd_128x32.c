...
//###################################################
/* NOTE : UTILISATION DE L'ECRAN LCD :

* Compléter la fonction  HAL_GPIO_WritePin dans stm32f4xx_hal_gpio.c
* initialisation dans main.c :  lcd_reset()
* affichage dans main.c :
 cls()
 lcd_printf("test LCD");
*/
//###################################################
...
static void wr_data(uint8_t *dat, uint32_t n)
{
	A0_SET;
	CS_CLR;
	HAL_SPI_Transmit_IT(&hspi1, dat, n);
	CS_SET;
}
